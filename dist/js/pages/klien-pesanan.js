$(document).ready(function () {
  ...
  function setTabel(response) {
    //counter untuk jumlah pesanan yg dimiliki user
    let counter = {
      Belumbayar: 0,
      Dibuat: 0,
      Dikirim: 0,
    ...
    if (typeof dataResponse === "object") {
      ...
      Object.keys(dataItem).forEach((element) => {
        ...
        //set tabel
        if (
          status["status"] == "1.menunggu info bank" ||
          status["status"] == "2.menunggu verifikasi bayar"
        ) {
          setDataTabel(
            $("#tabelBelumbayar"),
            gambar,
            namaProduk,
            dataPesanan[element]["item"],
            formatRupiah(jmlHarga, "Rp. "),
            status["warna"],
            status["status"],
            dataPesanan[element]["id_pesanan"]
          );
  ...
  function setDataTabel(
    ...
    tabel.append(
      $("<tr>")
        ...
        .append($("<td>").html(nama.slice(0, -4)).addClass("text-elipsis"))
        .append($("<td>").text(item).css("width", "50px"))
        .append($("<td>").text(harga))
        .append(
          $("<td>").append(
            $("<span>")
        ...
        .append(
          $("<td>").append(
            $("<a>")
              .append($("<span>").attr("data-feather", "eye"))
        ...
        $("#tabel" + element).append(
          $("<tr>").append(
            $("<td>")
              ...
              .append($("<p>").text("Belum Ada Pesanan"))
          )
        );
      }
    });
