...
$(document).ready(function () {
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response) {
            $.ajax({
              url:
                "/app/proses.php?api=telegram-update-status-pesanan&id=" +
                form.data("id"),
              type: "POST",
              data: $("#formBatalPesanan select").serialize(),
              // success: function (response) {
  ...
  function Init(idTarget) {
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response == "0") {
          alert(
            "ERROR ! : Terjadi Kesalahan dalam pengambilan data pesanan anda!"
          );
  ...
  function kondisiMenungguInfoBank() {
    $(".info-warning").html(
      "<div class='alert alert-info' role='alert'>Harap mengisi informasi pembayaran anda dengan klik tombol 'Bayar Sekarang' di bawah !!</div>"
    );
    $(".actionBtn").eq(0).html("Bayar Sekarang");
    $(".actionBtn").eq(1).html("Batalkan Pesanan");
    $("form#formBayar").attr(
      "action",
      "/app/proses.php?aksi=buat-pembayaran&pesanan=" +
        GetURLParameter("pesanan")
    );
  ...
  function kondisiMenungguVerifikasiBayar() {
    $(".info-warning").html(
      "<div class='alert alert-success' role='alert'>Silahkan cek cara pembayaran anda dengan klik tombol 'Cara Pembayaran'</div>\
  ...
  function kondisiPembuatan() {
    ...
    $(".actionBtn").eq(0).attr("data-target", "#pembayaran");
    $(".modal-content.pembayaran .modal-header .modal-title").html("");
    $(".modal-content.pembayaran .modal-body").html(
      "<div class='text-justify'>Barang Anda sedang dalam proses produksi, Harap menunggu dan cek secara berkala, dan pastikan alamat yang anda punya benar!!</div>"
    );
    $("#submitModal").hide();
    $("#konfirmasiBtn").hide();
    enableButtons();
  ...
  function kondisiPengiriman() {
    ...
    $(".modal-content.pembayaran .modal-body").html(
      "<div class='text-justify'><div class='text-justify'>Jika barang sudah sampai dan diinstalasi, Silahkan klik 'Barang telah diterima'</div>"
    );
    $("#submitModal").hide();
    $("#konfirmasiBtn").show();
    enableButtons();
          ...
          if (response == true) {
            $(".toast-body").html(
              "<div class='alert alert-success'>" +
                "Pesanan telah dikonfirmasi, Terima kasih telah berbelanja di Arture Furniture :)" +
                "</div>"
            );
            $(".toast").toast("show");
            $(".modal").modal("hide");
            ...
            $(".toast-body").html(
              "<div class='alert alert-danger'>" +
                "Terjadi kesalahan : Harap hubungi kami beserta Screenshot kesalahan!" +
                "</div>"
            );
            $(".toast").toast("show");
          }
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (response[0] == true) {
              $(".toast-body").html(
                "<div class='alert alert-success'>" +
                  "Info Bank telah ditambahkan, silahkan melanjutkan pembayaran pada bank yang dipilih!" +
                  "</div>"
              );
              $(".toast").toast("show");
              $(".modal").modal("hide");
