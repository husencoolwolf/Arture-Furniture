...
class database
{
  ...
  {
    $this->koneksi = mysqli_connect($this->hostname, $this->username, $this->password, $this->db, 3306);
  }
  ...
  function tambahKeranjang($idProduk, $idKlien, $quantity)
  {
    ...
    if ($sudahAda) {
      ...
      $query = "INSERT INTO keranjang values(DEFAULT, '$idKlien', '$idProduk', now(), $quantity)";
    }
  ...
  function tambahPesanan($data)
  {
    ...
    for ($i = 0; $i < count($produk); $i++) {
      $query2[] = "INSERT INTO detail_pesanan values(DEFAULT, '" . $data['id'] . "', '" . $produk[$i] . "', '" . $jumlah[$i] . "')";
    }
    ...
    if ($respon1) {
      ...
      if ($detailPesananSuccess === count($produk)) {
        $queryStatusAwal = "INSERT INTO status_pesanan VALUES (DEFAULT, '" . $data['id'] . "', 'menunggu info bank', now(), '');";
  ...
  function tambahInfoPembayaran($data)
  {
    ...
    if ($responMasuk) {
      $queryStatus = "INSERT INTO status_pesanan VALUES(DEFAULT, '" . $data['pesanan'] . "', 'menunggu verifikasi bayar', now(), '')";
  ...
  function tambahPesananAdmin($dataPesanan, $dataProduk)
  {
    ...
    if ($inputPesanan) {
      ...
      foreach ($dataProduk as $key => $value) {
        $queryProduk = "INSERT INTO detail_pesanan VALUES(DEFAULT, 
                '$idPesanan',
                '$key',
                '$value' )";
      ...
      if ($berhasilInputProduk == count($dataProduk)) {
        $queryStatusAwal = "INSERT INTO status_pesanan VALUES (DEFAULT, '" . $idPesanan . "', '" . $dataPesanan[2]['value'] . "', now(), 'input manual : " . $_SESSION['id_akun'] . "');";
        $responStatus = mysqli_query($this->koneksi, $queryStatusAwal);
        if ($responStatus) {
          return true;
        } else {
  ...
  function tambahPembayaranAdmin($data, $idPembayaran)
  {
    ...
    if ($inputPembayaran) {
      $queryStatus = "INSERT INTO status_pesanan VALUES(DEFAULT, '" . $data['selectPesanan'] . "', 'menunggu verifikasi bayar', now(), '" . $_SESSION['id_akun'] . "')";
      $responStatus = mysqli_query($this->koneksi, $queryStatus);
      if ($responStatus) {
        return true;
      } else {
  ...
  function tambahProjectAdmin($dataProject, $dataItem)
  {
    ...
    if ($inputProyek = mysqli_query($this->koneksi, $query1)) {
      $query2 = "INSERT INTO detail_proyek VALUES(DEFAULT, '$idBaru', '" . $dataProject[5]['value'] . "', '" . $dataProject[3]['value'] . "', '" . $dataProject[4]['value'] . "', '" . $dataProject[6]['value'] . "', '" . $dataProject[7]['value'] . "');";
      if (mysqli_query($this->koneksi, $query2)) {
        ...
        if ($dataBerhasilMasuk == count($dataItem)) {
          ...
          foreach ($ArrayIdItem as $key => $value) {
            $query4 = "INSERT INTO status_proyek VALUES(DEFAULT, '" . $value . "', 'menunggu hasil survey', now(), '" . $_SESSION['id_hak_akses'] . " : " . $_SESSION['id_akun'] . "', '0')";
        ...
        foreach ($dataItem as $k => $v) { // setiap id item yang ingin di 
          if (in_array($k, array_keys($itemSudahAda))) //cari id yg sama dari yang di update cek ke id yang sudah ada di db
          {
            ...
            if (mysqli_query($this->koneksi, $query)) {
              ...
              $query2 = "INSERT into status_proyek VALUES(DEFAULT, '$k', '" . $statusTerakhir['status'] . "', now(), '" . $_SESSION['id_hak_akses'] . " : " . $_SESSION['id_akun'] . "')";
              if (mysqli_query($this->koneksi, $query2)) {
                $dataBerhasilMasuk++;
              }
            }
  ...
  function updateStatusPesanan($data, $idPesanan)
  {
    ...
    $query = "INSERT INTO status_pesanan VALUES(DEFAULT, '$idPesanan', '$statusSelanjutnya', now(), '$keterangan')";
    if (mysqli_query($this->koneksi, $query)) {
      return true;
    } else {
      return "-1";
  ...
  function updateStatusProject($data, $idProject, $type)
  {
    ...
    if (!$idItem) {
      ...
      foreach ($idItem as $k => $v) {
        if (is_array($keterangan)) {
          $query = "INSERT INTO status_proyek VALUES(DEFAULT, '$v', '$statusSelanjutnya', now(), '$keterangan[$v]', '" . $progres[$v] . " | " . $_SESSION['id_akun'] . "')";
          if (mysqli_query($this->koneksi, $query)) {
            $dataBerhasilMasuk++;
          }
        } else {
          $query = "INSERT INTO status_proyek VALUES(DEFAULT, '$v', '$statusSelanjutnya', now(), '$keterangan', DEFAULT)";
          if (mysqli_query($this->koneksi, $query)) {
            $dataBerhasilMasuk++;
          }
        }
    ...
    $query = "INSERT INTO status_pesanan VALUES(DEFAULT, '$idProject', '$statusSelanjutnya', now(), '$keterangan')";
    if (mysqli_query($this->koneksi, $query)) {
      return true;
    } else {
      return "-1";
  ...
  {
    $query = "INSERT INTO status_pesanan VALUES(DEFAULT, '$idPesanan', 'selesai', now(), '')";
    if (mysqli_query($this->koneksi, $query)) {
      return true;
    } else {
      return false;
